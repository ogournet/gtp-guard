...
vty_create(struct thread_master *m, int vty_sock, sockaddr_t *addr)
{
	...
	vty_event(m, VTY_WRITE, vty_sock, vty);
	vty_event(m, VTY_READ, vty_sock, vty);
...
DEFUN(vty_line_listen,
      vty_line_listen_cmd,
      "listen (A.B.C.D|X:X::X:X) <1024-65535>",
      ...
      "TCP Port\n")
{
	...
	if (ret < 0) {
		vty_out(vty, "%% Error starting vty listener%s", VTY_NEWLINE);
		FREE(vty_listen_addr);
		return CMD_WARNING;
	}
...
DEFUN(no_vty_line_listen,
      no_vty_line_listen_cmd,
      "no listen (A.B.C.D|X:X::X:X) <1024-65535>",
      NO_STR
      "Launch telnet listener\n"
      "IPv4 Address\n"
      "IPv6 Address\n"
