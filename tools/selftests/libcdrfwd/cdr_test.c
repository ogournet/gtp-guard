...
_tick_cb(struct _thread *t)
{
	...
	tick_ev = task_add_timer(master, _tick_cb, NULL, USEC_PER_SEC);
}
...
_server_io_cb(struct _thread *ev)
{
	...
	srv->io = task_add_read(master, _server_io_cb, srv,
				fd, TIMER_DISABLED);
	return;
...
_server_accept(struct _thread *ev)
{
	...
	srv->io = task_add_read(master, _server_io_cb, srv, fd, TIMER_DISABLED);

	task_add_read(master, _server_accept, NULL, afd, TIMER_DISABLED);
}
...
fake_server(void)
{
	...
	for (i = 0; i < 3; i++) {
		...
		task_add_read(master, _server_accept, NULL, fd, TIMER_DISABLED);
