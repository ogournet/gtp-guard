...
#include "tools.h"
#include "traf_acl-def.h"

struct {
	__uint(type, BPF_MAP_TYPE_HASH);
	__uint(max_entries, 512);
	__type(key, struct traf_acl_key);
	__type(value, struct traf_acl_rule);
} traf_acl SEC(".maps");

struct traf_acl_data
{
...
static inline int
_acl_ipv4(struct traf_acl_key *acl_k, struct traf_acl_rule *out_rule)
{
	struct traf_acl_rule *r;

	bpf_printk("acl: searching if:%d vlan:%d gre:%x",
		   acl_k->ifindex, acl_k->vlan_id, acl_k->gre_remote);

	r = bpf_map_lookup_elem(&traf_acl, acl_k);
	if (r == NULL)
		return XDP_PASS;
 ...
 *
 * looks in 'traf_acl' a rule that can match incoming trafic.
 *
 ...
 */
static int
traf_acl_parse_pkt(struct xdp_md *ctx, struct traf_acl_data *d)
{
...
/*
 * rewrite packet according to 'traf_acl_rule'. usually the latest
 ...
 */
static int
traf_acl_rewrite_pkt(struct xdp_md *ctx, struct traf_acl_data *d)
{
	void *data = (void *)(long)ctx->data;
	void *data_end = (void *)(long)ctx->data_end;
	struct ethhdr *ethh = data;
