...
{
	struct gtp_apn *apn = vty->index;
	sockaddr_t *addr = &apn->nameserver;
	int ret;
...
{
	struct gtp_apn *apn = vty->index;
	sockaddr_t *addr = &apn->nameserver_bind;
	int ret, port;
      ...
      "IPv6 Address\n")
{
	...
	sockaddr_t *addr = &pco->ipcp_primary_ns;
	int ret;
      ...
      "IPv6 Address\n")
{
	struct gtp_apn *apn = vty->index;
	struct gtp_pco *pco = gtp_apn_pco(apn);
	sockaddr_t *addr = &pco->ipcp_secondary_ns;
	int ret;
