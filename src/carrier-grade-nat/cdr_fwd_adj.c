...
_server_connect_cb(struct _thread *ev)
{
	...
	sr->io = task_add_read(sr->ctx->cfg.loop, _server_io_cb, sr,
			       fd, TIMER_NEVER);
	thread_del_write(sr->io);
...
_server_reconnect(struct cdr_fwd_server *sr)
{
	...
	if (sr->connect_ev == NULL) {
		sr->connect_ev =
			task_add_timer(sr->ctx->cfg.loop, _server_connect,
...
static int
_set_fd_block(int fd, bool blocking)
{
	long val;
...
_server_connect(struct _thread *ev)
{
	...
	thread_del_timer(sr->connect_ev);
	sr->connect_ev = NULL;
	...
	if (_set_fd_block(fd, false) < 0) {
		err(sr->ctx->log, "fcntl[O_NONBLOCK]: %m");
		close(fd);
		_server_reconnect(sr);
		return;
	}
	...
	if (!r) {
		...
		sr->io = task_add_read(sr->ctx->cfg.loop, _server_io_cb,
					 sr, fd, TIMER_NEVER);
		sr->flags |= CDR_FWD_FL_CONNECTED;
		cdr_fwd_remote_connected(sr);
		_server_sm(sr);
		...
		trace2(sr->ctx->log, "%s: connection in progress...",
		       sr->addr_str);
		sr->io = task_add_write(sr->ctx->cfg.loop, _server_connect_cb,
					 sr, fd, 8 * USEC_PER_SEC);
	}
}
...
{
	task_add_event(sr->ctx->cfg.loop, _server_connect, sr, 0);
}
