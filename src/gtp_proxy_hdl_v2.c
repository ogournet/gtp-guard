		...
		struct gtp_session *s, struct gtp_ie_eps_bearer_id *bearer_id)
{
	 ...
	 * create a new VTEID */
	if ((*f_teid->ipv4 == ((struct sockaddr_in *) &srv_gtpc_ingress->s.bind_addr)->sin_addr.s_addr) ||
	    (*f_teid->ipv4 == ((struct sockaddr_in *) &srv_gtpc_egress->s.bind_addr)->sin_addr.s_addr)) {
